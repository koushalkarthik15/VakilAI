...
Vakil AI integrates multiple Large Language Models (LLMs) specifically for semantic interpretation and natural language generation, adhering strictly to a **"reasoning only"** boundary:

1. **Google Gemini (Semantic Extraction & Preparation Generation)**:
   - **Stage 1 (Understand)**: Extracts structured factual data (parties, dates, obligations, clause mapping) from the raw PDF text. Gemini shines in parsing long, unstructured legal language into rigid JSON objects.
   - **Stage 5 (Prepare)**: Generates the human-readable attorney preparation briefing sheet, weaving together deterministic legal findings into a cohesive narrative requested by the user.

2. **Groq / Llama 3 (Semantic Reasoning & Comparison)**:
   - **Stage 2 & 3 (Flag & Compare)**: Used for fast, nuanced semantic comparisons where deterministic matching falls short. Groq analyzes the extracted contract clauses against the MongoDB legal rules to detect subtle deviations, ambiguities, and potential legal risks.

**Strict GenAI Guardrails:**
- **Zero Prompt Injection Tolerance**: Extracted text is heavily sanitized.
- **Zod Validation**: Every single JSON response generated by Gemini and Groq is parsed against strict Zod schemas before state transition.
- **No Hallucinated Citations**: The AI models are completely restricted from generating their own legal sources. They are only allowed to reference exact `source_ids` provided to them from the deterministic MongoDB Legal KB.
